...
int main(){
  ...
  */
  ...
  pthread_create( &thr1, NULL, write_test, (void*)(&write_data));//host toFPGA 
  pthread_create( &thr2, NULL, read_test, (void*)(&read_data));//FPGA-GPU 
  ...
  cuMemcpyDtoH(&arr2[0], (unsigned long long)dptr, lock.size); //GPU -HOST
  for (int i = 0; i < 30; i++){
    printf("%d -> %d\n",arr1[i], arr2[i]);
  }
