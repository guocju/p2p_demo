...
//DMA中分配buffer
  ...
  */
  ...
  int test_bytes = 16 * 1024 * 1024 ;
  read( fd_i, &arr1[0], n_byte); 
// write( fd_o, &arr1[0], n_byte);  
  int rc;
  auto start = std::chrono::system_clock::now();
  for(int i = 0; i <10; i ++){
    //write( fd_o, &arr1[0], test_bytes);  
    //lseek(fd_o, 0, SEEK_SET);
    //read( fd_i, &arr1[0], n_byte); 
    read( fd_i, buffer, n_byte); 
  }
 // printf("%d\n\r", arr2[test_bytes/4-1]);
  ...
  std::cout << msec << " ms. " << "\n";
  double bandwidth = static_cast<double>(N*4)*10 / msec * 1000 / (1024 * 1024 * 1024);    
