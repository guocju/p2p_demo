...
int main(){
  ...
  */
  ...
  //write_data = { fd_o, IOCTL_XDMA_GPU_WRITE, &lock};
  //read_data = { fd_i, &arr2[0], n_byte};
  /*
  ...
  */
  //for(int i = 0; i < 100; i++){
  ioctl( fd_o, IOCTL_XDMA_GPU_WRITE, &lock);//如果不运行ioctl   仍然可以从主机（CPU）上直接搬运数据到FPGA
  ...
  */
  ...
  // do_free_attr:
  // flag = 0;
  printf("calling cuPointerSetAttribute");
  //cuPointerSetAttribute(&flag, CU_POINTER_ATTRIBUTE_SYNC_MEMOPS, dptr);
  
 do_free_memory:
 printf("calling do_free_memory");
  //cuMemFree(dptr);
  
 do_free_context:
 printf("calling do_free_context");
  ...
  ioctl(fd_o, IOCTL_XDMA_GPU_FREE, &lock);
  return 0;
}
