...
#define MAX_DESC_COUNT 1000  // Define a maximum descriptor count

// Global variables to hold the saved descriptors and count

static int desc_count = 0;
static struct desc_array *global_desc_array = NULL;
static uint32_t global_desc_num = 0;
/* Module Parameters */
...
struct desc_array *desc_array;  // 全局变量声明
...
void print_xdma_desc(struct xdma_desc *desc) {
    printk(KERN_INFO "Descriptor details:\n");
    printk(KERN_INFO "  bytes: 0x%x\n", desc->bytes);
    printk(KERN_INFO "  src_addr_lo: 0x%x\n", desc->src_addr_lo);
    printk(KERN_INFO "  src_addr_hi: 0x%x\n", desc->src_addr_hi);
    printk(KERN_INFO "  dst_addr_lo: 0x%x\n", desc->dst_addr_lo);
    printk(KERN_INFO "  dst_addr_hi: 0x%x\n", desc->dst_addr_hi);
	printk(KERN_INFO "  next_addr_lo: 0x%x\n", desc->next_lo);
    printk(KERN_INFO "  next_addr_hi: 0x%x\n", desc->next_hi);
}
			...
			struct xdma_request_cb *req, struct xdma_transfer *xfer)
{
	...
	for (i = 0; i < xfer->desc_num; i++) {
		...
		//print_xdma_desc(xfer->desc_virt + i);
	}
	struct desc_array *desc_array = kmalloc(sizeof(struct xdma_desc) * xfer->desc_num, GFP_KERNEL);
	if (!desc_array) {
		pr_err("Failed to allocate memory for desc_array.\n");
		spin_unlock_irqrestore(&engine->lock, flags);
		return -ENOMEM;
	}
/*
	for (i = 0; i < xfer->desc_num; i++) {
		...
		//dbg_desc("set next adj at index %d to %u\n", i, next_adj);
		//xdma_desc_adjacent(xfer->desc_virt + i, next_adj);
		desc_array[i].src_addr_lo = (xfer->desc_virt + i)->src_addr_lo;
		desc_array[i].src_addr_hi = (xfer->desc_virt + i)->src_addr_hi;
		desc_array[i].dst_addr_lo = (xfer->desc_virt + i)->dst_addr_lo;
		desc_array[i].dst_addr_hi = (xfer->desc_virt + i)->dst_addr_hi;
		//desc_array[i].next_lo = (xfer->desc_virt + i)->next_lo;
		//desc_array[i].next_hi = (xfer->desc_virt + i)->next_hi;
		desc_array[i].bytes = (xfer->desc_virt + i)->bytes;
		//desc_array[i].control = (xfer->desc_virt + i)->control;
		//自己添加打印XDMA＿DESC描述符
		//print_xdma_desc(xfer->desc_virt + i);
		    pr_info("desc_array[%d]: src_addr_lo=0x%lx, src_addr_hi=0x%lx, "
            "dst_addr_lo=0x%lx, dst_addr_hi=0x%lx, bytes=%u, next_lo=0x%lx, next_hi=0x%lx, control=0x%lx\n", 
            i,
            desc_array[i].src_addr_lo, desc_array[i].src_addr_hi, 
            desc_array[i].dst_addr_lo, desc_array[i].dst_addr_hi,
            desc_array[i].bytes, (xfer->desc_virt + i)->next_lo, 
			(xfer->desc_virt + i)->next_hi, (xfer->desc_virt + i)->control);
		pr_info("desc_virt = %p, desc_bus = 0x%llx\n", xfer->desc_virt + i , (u64)xfer->desc_bus + i);

	}
*/
	// 保存到全局变量
	global_desc_array = desc_array;
	global_desc_num = xfer->desc_num;
	
	spin_unlock_irqrestore(&engine->lock, flags);
	return 0;
}

static int transfer_init_byp(struct xdma_engine *engine,
			struct xdma_request_cb *req, struct xdma_transfer *xfer)
{
	...
	u32 control;
	unsigned long flags;
	struct vm_area_struct *vma;  // 声明 vma
	memset(xfer, 0, sizeof(*xfer));
	...
	for (i = 0; i < xfer->desc_num; i++) {
		...
		//print_xdma_desc(xfer->desc_virt + i);
		    pr_info("desc_array[%d]: src_addr_lo=0x%lx, src_addr_hi=0x%lx, "
            "dst_addr_lo=0x%lx, dst_addr_hi=0x%lx, bytes=%u\n", 
            i,
            desc_array[i].src_addr_lo, desc_array[i].src_addr_hi, 
            desc_array[i].dst_addr_lo, desc_array[i].dst_addr_hi,
            desc_array[i].bytes);
...
/*//自己
ssize_t xdma_xfer_desc_byp(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
    struct xdma_transfer *xfer;
    struct desc_array *desc_array;
    int i, num_desc;
    ssize_t ret = 0;

    // Step 1: Get the number of descriptors you want to copy to user space
    num_desc = get_desc_count();  // Function to retrieve the count of descriptors

    // Step 2: Allocate memory for desc_array
    desc_array = kmalloc(sizeof(struct desc_array) * num_desc, GFP_KERNEL);
    if (!desc_array) {
        pr_err("Failed to allocate memory for desc_array.\n");
        return -ENOMEM;
    }

    // Step 3: Fill the desc_array with data (same as in your transfer_init_byp function)
    for (i = 0; i < num_desc; i++) {
        desc_array[i].src_addr_lo = get_src_addr_lo(i);  // Assuming these functions get the data
        desc_array[i].src_addr_hi = get_src_addr_hi(i);
        desc_array[i].dst_addr_lo = get_dst_addr_lo(i);
        desc_array[i].dst_addr_hi = get_dst_addr_hi(i);
        desc_array[i].bytes = get_bytes(i);
    }

    // Step 4: Copy the desc_array from kernel space to user space
    if (count < sizeof(struct desc_array) * num_desc) {
        pr_err("User buffer too small.\n");
        ret = -EINVAL;
        goto free_desc_array;
    }

    if (copy_to_user(buf, desc_array, sizeof(struct desc_array) * num_desc)) {
        pr_err("Failed to copy data to user space.\n");
        ret = -EFAULT;
        goto free_desc_array;
    }

    ret = sizeof(struct desc_array) * num_desc;  // Return the number of bytes copied to user space
...
*/
/*
static int get_desc_array(struct xdma_transfer *xfer, struct xdma_desc_array_ioctl *desc_array_ioctl) {
    // 检查输入参数
    if (!xfer || !desc_array_ioctl) {
        pr_err("Invalid input parameters.\n");
        return -EINVAL;  // 输入无效
    }

    // 为传递到用户空间的描述符数组分配内存
    desc_array_ioctl->desc_count = xfer->desc_num;
    desc_array_ioctl->desc_array = kmalloc(sizeof(struct xdma_desc) * xfer->desc_num, GFP_KERNEL);
    if (!desc_array_ioctl->desc_array) {
        pr_err("Failed to allocate memory for desc_array.\n");
    ...
    // 确保 desc_virt 指针有效
    if (!xfer->desc_virt) {
        pr_err("Invalid descriptor pointer.\n");
        kfree(desc_array_ioctl->desc_array);  // 释放已分配的内存
        return -EINVAL;  // 无效的描述符指针
    }

    // 将描述符数据从 xfer->desc_virt 复制到 desc_array
    memcpy(desc_array_ioctl->desc_array, xfer->desc_virt, sizeof(struct xdma_desc) * xfer->desc_num);

    // 将内核中的描述符数组复制到用户空间
    int rv = copy_to_user((struct xdma_desc_array_ioctl __user *)arg, desc_array_ioctl, sizeof(struct xdma_desc_array_ioctl));
    if (rv != 0) {
        pr_err("Failed to copy descriptor array to user space\n");
        kfree(desc_array_ioctl->desc_array);  // 释放已分配的内存
        return -EFAULT;
    }
...
static int get_desc_count(struct xdma_transfer *xfer) {
    if (!xfer) {
        pr_err("Invalid xfer pointer.\n");
        return -EINVAL;  // 输入无效
    }

    // 返回描述符的数量
    return xfer->desc_num;
}
...
static void print_desc_array(struct xdma_request_cb *req)
{
    int i;
    // 遍历所有描述符
    for (i = 0; i < req->sw_desc_cnt; i++) {
        // 打印每个描述符的物理地址和长度信息
        pr_info("desc[%d]: addr=0x%llx, len=%u\n",
                i,
                (unsigned long long)req->sdesc[i].addr,
                req->sdesc[i].len);
    }
}
			 ...
			 struct sg_table *sgt, bool dma_mapped, int timeout_ms)
{
	...
	//printk(KERN_INFO "caling xdma_xfer_submit\n");
	//printk(KERN_INFO "xdma_xfer_submit\n");
	//printk(KERN_INFO "xdma_xfer_submit: dev_hndl=%p, channel=%d, write=%d, ep_addr=0x%llx, dma_mapped=%d, timeout_ms=%d\n",
	...
	if (write == 1) {
		...
		printk(KERN_INFO "Using H2C channel %d\n", channel);
		...
		printk(KERN_INFO "Using C2H channel %d\n", channel);
	...
	printk(KERN_INFO "Engine magic: %lx, expected magic: %lx\n", engine->magic, MAGIC_ENGINE);
	...
	/* check the direction */
    //printk(KERN_INFO "Checking DMA direction: engine direction=%d, request direction=%d\n", engine->dir, dir);
	if (engine->dir != dir) {
	...
	if (!dma_mapped) {
		printk(KERN_INFO "Mapping scatterlist entries\n");
		nents = pci_map_sg(xdev->pdev, sg, sgt->orig_nents, dir);
		if (!nents) {
			pr_info("map sgl failed, sgt 0x%p.\n", sgt);
			return -EIO;
		}
		printk(KERN_INFO "Scatterlist mapped, nents=%d\n", nents);
		sgt->nents = nents;
	} else {
		printk(KERN_INFO "DMA already mapped, checking sgt entries\n");
		if (!sgt->nents) {
	...
	while (nents) {
		...
		/* build transfer */
		printk(KERN_INFO "Initializing transfer %d\n", tfer_idx);
		...
		nents -= xfer->desc_num;
		if (!nents) {
			printk(KERN_INFO "Last transfer in request\n");
...
unmap_sgl:
	if (!dma_mapped && sgt->nents) {
		...
		printk(KERN_INFO "unmap_sgl success!\n");
		pr_info("Calling pci_unmap_sg with device: %p, sgl: %p, nents: %d, dir: %d\n",
            xdev->pdev, sgt->sgl, sgt->orig_nents, dir);
		sgt->nents = 0;
		pr_info("xdma_xfer_submit called pci_unmap_sg.\n");
	}
	// printk("submit0");
	if (req)
		xdma_request_free(req);
...
//自己增加版本
ssize_t xdma_dsc_byp_xfer(void *dev_hndl, int channel, bool write, u64 ep_addr,
                         void __user *user_buf,  // 改为用户空间指针
                         bool dma_mapped, int timeout_ms)
{
    ...
    ssize_t done = 0;
    // 打印调试信息
    pr_info("User buffer address: %p, desc_num: %d\n", user_buf, global_desc_num);
    pr_info("Size to copy: %zu bytes\n", sizeof(struct xdma_desc) * global_desc_num);


    // 打印数组内容
    /*for (i = 0; i < global_desc_num; i++) {
        pr_info("Saved desc_array[%d]: src_addr_lo=0x%lx, src_addr_hi=0x%lx, "
                "dst_addr_lo=0x%lx, dst_addr_hi=0x%lx, bytes=%u\n",
                i,
                global_desc_array[i].src_addr_lo,
                global_desc_array[i].src_addr_hi,
                global_desc_array[i].dst_addr_lo,
                global_desc_array[i].dst_addr_hi,
                global_desc_array[i].bytes);
    }*/
    // 打印详细的调试信息
    pr_info("Debug info:\n");
    pr_info("  User buffer address: %p\n", user_buf);
    pr_info("  Descriptor count: %d\n", global_desc_num);
    pr_info("  Single desc size: %zu bytes\n", sizeof(struct desc_array));
    ...
    if (!global_desc_array) {
        pr_err("Invalid descriptor array\n");
        return -EINVAL;
    }

    // 验证用户空间地址访问权限
    if (!access_ok(user_buf, sizeof(struct xdma_desc) * global_desc_num)) {
        pr_err("Invalid user space address range\n");
        return -EFAULT;
    }
    pr_info(" valid user buf");
    if (copy_to_user(user_buf, global_desc_array, sizeof(struct xdma_desc) * global_desc_num)) {
        pr_err("Failed to copy to user space\n");
        return -EFAULT;
    }
    pr_info(" copy_to_user successn");
    // 9. 确保内存操作完成
    mb();

    if (global_desc_array) {
        kfree(global_desc_array);
        global_desc_array = NULL;
    }
...
ssize_t xdma_dsc_byp(void *dev_hndl, int channel, bool write, u64 ep_addr,
			struct sg_table *sgt, bool dma_mapped, int timeout_ms){
	struct descriptor *saved_descs;
	int desc_index = 0;
	struct xdma_dev *xdev = (struct xdma_dev *)dev_hndl;
	struct xdma_engine *engine;
	int rv = 0, tfer_idx = 0, i;
	...
	enum dma_data_direction dir = write ? DMA_TO_DEVICE : DMA_FROM_DEVICE;
	struct xdma_request_cb *req = NULL;
	//struct xdma_desc *desc_array;  // or array of descriptors
	struct xdma_transfer *xfer;    // or another suitable type for xfer

	printk(KERN_INFO "start xdma_dsc_byp mode!\n");
//用于存储descriptor的FPGA地址
	if (!dev_hndl)
		return -EINVAL;
	...
	while (nents) {
		...
		// build transfer 
		rv = transfer_init_byp(engine, req, &req->tfer[0]);
		if (rv < 0) {
			mutex_unlock(&engine->desc_lock);
			goto unmap_sgl;
		}
		...
		if (!nents) {
			xfer->last_in_request = 1;
			xfer->sgt = sgt;
			pr_info("All descriptors generated successfully.\n");
		}
...
// 直接将描述符保存到数组
	//struct xdma_desc *desc_array;
	//int num_descs = xfer->num_descs;  // 假设 num_descs 已经初始化
    // 使用 kmalloc 分配内存
		pr_info("desc_virt = %p, desc_bus = 0x%llx\n", xfer->desc_virt + i , (u64)xfer->desc_bus + i);
...
unmap_sgl:
	pr_info("Calling unmap_sgl with device: %p, sgl: %p, nents: %d, dir: %d\n",
		xdev->pdev, sgt->sgl, sgt->nents, dir);
	pr_info( "xdma_dsc_byp: dma_mapped=%d\n" , dma_mapped);
	if (!dma_mapped && sgt->nents) {
		for (i = 0; i < sgt->nents; i++) {
			pr_info("sgl[%d]: dma_address = %pad, length = %u\n", 
					i, &sgt->sgl[i].dma_address, sgt->sgl[i].length);
		}
		...
		pr_info("xdma_dsc_byp called pci_unmap_sg.\n");
	}
	// printk("submit0");
	if (req)
		xdma_request_free(req);
...
/*自己增加 废弃代码
void save_desc_to_array(struct xdma_descriptor *desc_chain, int num_descs)
{
    int i;

    // 确保我们不会超过预定义的最大数量
    if (desc_count + num_descs > MAX_DESC_COUNT) {
        pr_err("Error: Exceeded maximum descriptor count.\n");
        return;
    }

    // 遍历描述符链并保存到全局数组
    for (i = 0; i < num_descs; i++) {
        saved_descs[desc_count + i] = desc_chain[i];  // 将描述符保存到全局数组
    }

    // 更新当前保存的描述符数量
    desc_count += num_descs;

    pr_info("Saved %d descriptors. Total saved: %d\n", num_descs, desc_count);
}
void print_saved_descs()
{
    int i;
    pr_info("Total saved descriptors: %d\n", desc_count);
    for (i = 0; i < desc_count; i++) {
        pr_info("Descriptor %d: DMA Address = 0x%llx, Length = %u\n", 
                i, saved_descs[i].dma_address, saved_descs[i].length);
    }
}
*/
...
unmap_sgl:
	if (!dma_mapped && sgt->nents) {
		pci_unmap_sg(xdev->pdev, sgt->sgl, sgt->orig_nents, dir);
		sgt->nents = 0;
		pr_info("xdma_xfer_completion called pci_unmap_sg.\n");
	}
...
unmap_sgl:
	if (!dma_mapped && sgt->nents) {
		pci_unmap_sg(xdev->pdev, sgt->sgl, sgt->orig_nents, dir);
		sgt->nents = 0;
		pr_info("xdma_xfer_submit_nowait called pci_unmap_sg.\n");
	}
