...
uint64_t getopt_integer(char *optarg) {
  ...
  if (rc <= 0) rc = sscanf(optarg, "%lu", &value);
  // printf("sscanf() = %d, value = 0x%lx\n", rc, value);

  return value;
}

ssize_t read_to_buffer(void *fname, int fd, char *buffer, uint64_t size,
                       uint64_t base) {
  ssize_t rc;
  uint64_t count = 0;
  char *buf = buffer;
...
ssize_t write_from_buffer(void *fname, int fd, char *buffer, uint64_t size,
                          uint64_t base) {
  ssize_t rc;
  uint64_t count = 0;
  char *buf = buffer;
