...
# input
indptr_tvm = tvm.nd.array(indptr, device=dev)
indices_tvm = tvm.nd.array(indices, device=dev)
data_tvm = tvm.nd.array(data, device=dev)

# output buffer 分配
row_idx_s = tvm.nd.empty((row_s,), dtype="int32", device=dev)
col_idx_s = tvm.nd.empty((short_align_val, row_s), dtype="int32", device=dev)
val_s = tvm.nd.empty((short_align_val, row_s), val_dtype, device=dev)
row_idx_l = tvm.nd.empty((row_l,), dtype="int32", device=dev)
col_idx_l = tvm.nd.empty((row_l, long_align_val), dtype="int32", device=dev)
val_l = tvm.nd.empty((row_l, long_align_val), val_dtype, device=dev)

ex["fpga_process"](
    indptr_tvm, indices_tvm, data_tvm,
    row_idx_s, col_idx_s, val_s,
    row_idx_l, col_idx_l, val_l
)

print("row_idx_s:", row_idx_s.numpy())
print("col_idx_s:", col_idx_s.numpy())
print("val_s:", val_s.numpy())
print("row_idx_l:", row_idx_l.numpy())
print("col_idx_l:", col_idx_l.numpy())
print("val_l:", val_l.numpy())